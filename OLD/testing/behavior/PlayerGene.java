...
public class PlayerGene {
    ...
    private final Map<PlayerAICard.Bias, Double> playerCardBias = new HashMap<PlayerAICard.Bias, Double>(
	    PlayerAICard.Bias.values().length);
	...
	// card rating biases
	for (PlayerAICard.Bias bias : PlayerAICard.Bias.values()) {
	...
	out.append("\t---Cards:\n");
	for (PlayerAICard.Bias bias : this.playerCardBias.keySet()) {
	    out.append(String.format("\t%s: %.1f\n", bias,
    ...
    public Map<PlayerAICard.Bias, Double> getPlayerCardBias() {
